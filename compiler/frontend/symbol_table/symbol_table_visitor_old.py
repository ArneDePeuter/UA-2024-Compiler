...
class SymbolTableVisitor(AstVisitor):
    ...
    def get_expression_type(self, node):
        if isinstance(node, ast.INT):
            ...
            if node.operator == node.Operator.ADDRESSOF:
                ...
                new_address_qualifiers = copy.deepcopy(operand_type.address_qualifiers)
                new_address_qualifiers.append(ast.AddressQualifier.pointer)
                return Type(base_type=operand_type.base_type, line=node.line, position=node.position, const=operand_type.const, address_qualifiers=new_address_qualifiers)
            elif node.operator == node.Operator.DEREFERENCE:
                # Decrease pointer level for dereference operation.
                new_address_qualifiers = copy.deepcopy(operand_type.address_qualifiers)
                new_address_qualifiers.remove(ast.AddressQualifier.pointer)
                return Type(base_type=operand_type.base_type, line=node.line, position=node.position, const=operand_type.const, address_qualifiers=new_address_qualifiers)
            else:
                return Type(base_type=operand_type.base_type, line=node.line, position=node.position, const=operand_type.const, address_qualifiers=operand_type.address_qualifiers)
        else:
    ...
    def visit_variable_declaration(self, node: ast.VariableDeclaration):
        ...
        for qualifier in node.qualifiers:
            ...
            # TODO: Hande constness for pointers
            if len(node.var_type.address_qualifiers) > 0 and node.var_type.const:
                while True:
                    referenced_symbol = self.symbol_table.lookup(qualifier.initializer.value.name, current_scope_only=True)
                    referenced_symbol.type.const = True
    ...
    def visit_assignment_statement(self, node: ast.AssignmentStatement):
        ...
        if isinstance(node.left, ast.UnaryExpression):
            identifier = node.left.value.name
        else:
            identifier = node.left.name
        symbol = self.symbol_table.lookup(identifier)
        if symbol is None:
            raise SemanticError(f"Use of undeclared variable '{identifier}'.", node.line, node.position)

        # Check if the variable is declared as const
        if symbol.type.const:
            raise SemanticError(f"Cannot assign to const variable '{identifier}'.", node.line, node.position)

        # Optionally, check if the assignment's value type is compatible with the variable's type
        value_type = self.get_expression_type(node.right)
        symbol_type = self.get_expression_type(node.left)

        # Here i raise warning that i discart the const qualifier
        if symbol_type.const:
            warning = WarningError(f"Initializing {symbol_type} with {value_type} discard the const qualifier.",
                                   value_type.line, value_type.position)
            warning.warn()

        if not self.is_type_compatible(symbol_type, value_type):
            raise SemanticError(
                f"Type mismatch in assignment to '{identifier}': cannot assign {value_type} to {symbol_type}.",
                node.line, node.position)
